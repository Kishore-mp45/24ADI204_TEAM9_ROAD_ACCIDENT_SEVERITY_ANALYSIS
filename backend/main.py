...
app = FastAPI(title="Road Accident Severity API", lifespan=lifespan)
...
@app.get("/health")
def health():
    return {"status": "ok", "data_loaded": global_df is not None}
...
def feature_hist(feature: str):
    ...
    fig = px.histogram(global_df, x=feature, nbins=50, title=f"{feature} Density & Spread")
    fig.update_layout(margin=dict(l=20, r=20, t=40, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
    return Response(content=fig.to_json(), media_type="application/json")
...
def v2_wind_hist():
    ...
    col_data = col_data[col_data <= col_data.quantile(0.99)]
    counts, bin_edges = np.histogram(col_data, bins=40)
    data = []
    for i in range(len(counts)):
...
# =====================================================================
# Static files — only mount if the build directory actually exists
# =====================================================================
...
# If neither directory exists (e.g. backend-only deploy on Render), skip mounting.
# The /health endpoint above confirms the API is running.
